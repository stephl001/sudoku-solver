module Sudoku(parseBoard,rows,cols,boxes,Board,Row,Cell,CellValue,validBoard) where

import Data.Char (digitToInt)
import Data.List (transpose,nub,(\\),subsequences,intersect)
...
findNakedGroups size row = [group | group <- subsequences row, length group == size, allEqual group, length (head group) == size]

-- Function to find all hidden groups of a given size in a row
findHiddenGroups :: Int -> Row -> [[CellValue]]
findHiddenGroups size row = [group | group <- subsequences [minBound .. maxBound], length group == size, countOccurrences group row == size]
...
countOccurrences group row = length [cell | cell <- row, not (null (cell `intersect` group))]

-- Function to prune a row based on naked and hidden groups
pruneRowSized :: Int -> Row -> Row
pruneRowSized size row = foldl pruneCell (foldl pruneCellHidden row hiddenGroups) nakedGroups
  where
    nakedGroups = nub $ findNakedGroups size row
    hiddenGroups = nub $ findHiddenGroups size row
    pruneCell r group = map (\cell -> if cell `notElem` group then cell \\ (head group) else cell) r
    pruneCellHidden r group = map (\cell -> if not (null (cell `intersect` group)) && length cell > size then cell `intersect` group else cell) r

pruneRow :: Int -> Row -> Row
pruneRow maxSize row = foldl (flip pruneRowSized) row [1..maxSize]

sampleRow :: Row
sampleRow = [[Eight],[Six],allValues,[Seven],[Nine],[Five],[One],allValues,[Four]]

nakedPairsRow :: Row
nakedPairsRow = [[One,Three,Seven],[Two],[Five],[Three,Six,Eight],[Three,Four,Eight],[One,Seven],[Three,Four,Six],[Nine],[One,Seven]]

hiddenPairsRow :: Row
hiddenPairsRow = [[Three,Eight],[Two],[Three,Six],[Three,Four,Six],[Five],[Three,Four],[One,Seven,Eight],[Nine],[One,Six,Seven]]

nakedTripsRow :: Row
nakedTripsRow = [[One,Four,Seven],[One,Seven,Eight],[Two],[Five],[Three,Six,Eight],[One,Seven,Eight],[Nine],[Three,Four,Six],[One,Seven,Eight]]
...
pruneBoard maxSubgroupSize board = 
  let newBoard = foldr (pruneTransformedBoard maxSubgroupSize) board [rows,cols,boxes]
  in if newBoard == board then board else pruneBoard maxSubgroupSize newBoard
...
pruneTransformedBoard maxSubgroupSize extractRows = (extractRows . (pruneRows maxSubgroupSize) . extractRows)
